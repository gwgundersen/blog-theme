...
{% katexmm %}
Given a dataset $X^n \triangleq \{X_1, \cdots, X_n\}$ drawn from an unknown distribution $Q$. Under the exchangeability assumption, one could model the unknown distribution $Q$ using a statistical model with likelihood function $p(X^n|w)$, a prior $\varphi(w), w\in W \subseteq \mathbb{R}^d$. Using Bayes rule, the posterior of the parameter given the data is
